...
struct BFSNode
{
    ...
    BFSNode* prev;
    BFSNode():prev(NULL){}
    BFSNode(u64 _id, u64 _level, BFSNode* _prev):id(_id), level(_level), prev(_prev){}
};
...
struct PageR // 用于PageRank
{
    double *pageRankVec1;
    double *pageRankVec2;
    u64 flag;
    PageR():pageRankVec1(NULL), pageRankVec2(NULL), flag(1){};
};
...
public:
    /*
        friend函数都是多线程的工作函数
    */
    friend void PageRank_Fun(void* x);
    friend void BFS_Fun(void* x);
    ...
    void TestRun(int mod, string fn, string objfn); // 用于批量测试
...
private:
    ...
    void TestSerch(int mod, string obj); // 用于批量测试
