...
void pSAscan1(std::string input_filename, std::string output_filename,
    std::string gap_filename, long ram_use, long max_threads,
    bool verbose, long *SA,long gap_buf_size = (1L << 21)) {
  long n_gap_buffers = 2 * max_threads;
  if (ram_use < 6L) {
    fprintf(stderr, "Error: not enough memory to run pSAscan.\n");
    std::exit(EXIT_FAILURE);
...
//  if (max_block_size < (1L << 31)) {
  if(0){
    ...
    std::vector<half_block_info<uint40> > hblock_info = partial_sufsort<uint40>(input_filename,
        output_filename, gap_filename, length, max_block_size, ram_use, max_threads, gap_buf_size, verbose);
    merge1<uint40>(output_filename, ram_use, hblock_info,SA);
  }
  long double total_time = utils::wclock() - start;
...
void pSAscan1(std::string input_filename, std::string output_filename,
    std::string gap_filename, long ram_use, long max_threads, bool verbose,long *SA) {
  psascan_private::pSAscan1(input_filename, output_filename,
      gap_filename, ram_use, max_threads, verbose,SA);
}
