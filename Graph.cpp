...
struct PageRank_Arg
{
    ...
    volatile u64 *cnt;
    Graph* ptr;
    PageRank_Arg(double *_preVec, double *_nowVec, MutexLock* _tasklock, volatile u64 *_cnt, Graph* _ptr)
            :preVec(_preVec), nowVec(_nowVec), tasklock(_tasklock), cnt(_cnt), ptr(_ptr){}
};

void PageRank_Fun(void* x)
{
    PageRank_Arg* arg = (PageRank_Arg*)x;
    u64 *preRes = NULL;
    u64 preNums, preSt;
    for(u64 i = arg->st;i < arg->st + arg->n;++i){
        arg->ptr->Adj->GetPrevVid(i, preRes, preNums, preSt);
        double sum = 0;
        for(u64 j = 0;j < preNums;++j){
            u64 nextNums = arg->ptr->Adj->GetOutNums(preRes[j]);
            sum = sum + arg->preVec[preRes[j]] / nextNums;
        }
        arg->nowVec[i] = (1 - arg->d) + arg->d * sum;
    }
    if(preRes)
        delete [] preRes;
...
struct BFS_Arg
{
    ...
    BFSQue *BFSQ;
    BFSNode *tmp;
    BFS_Arg(BFSQue *_BFSQ, BFSNode *_tmp, volatile u64 *_cnt, Graph* _ptr, MutexLock *_tasklock, InArray* _vis)
        :BFSQ(_BFSQ), tmp(_tmp), cnt(_cnt), ptr(_ptr), tasklock(_tasklock), vis(_vis){}
};

void BFS_Fun(void* x)
{
    BFS_Arg* arg = (BFS_Arg*)x;
    u64 *BFSRes = NULL;
    u64 BFSNums, BFSSt;
    u64 tmpIdx = 0;
    for(u64 i = arg->st;i < arg->st + arg->n;++i){
        arg->ptr->Adj->GetNextVid(arg->BFSQ->vec[i].id, BFSRes, BFSNums, BFSSt);
        for(u64 j = 0;j < BFSNums;++j){
            if(!arg->vis->GetValue(BFSRes[j])){
                new (&arg->tmp[tmpIdx]) BFSNode(BFSRes[j], arg->BFSQ->vec[i].level + 1, &arg->BFSQ->vec[i]);
                ++tmpIdx;
            }
        }
...
{
    /* 其他预处理 */
    max_threads = (long)omp_get_max_threads() - 1;
...
void Graph::TestRun(int mod, string fn, string objfn)
{

// //测试person平均度
// u64 personN = vkinds["person"].n, personSt = vkinds["person"].st;
// u64 sum = 0;
// u64 *res = NULL, nums = 0, st = 0;
// for(u64 i = vkinds["person"].st;i <vkinds["person"].st + personN;++i){
//     Adj->GetNextVid(i, res, nums, st);
//     for(u64 j = 0;j < nums;++j){
//         if(res[j] >= personSt && res[j] < personSt + personN)
//             ++sum;
//         else if(res[j] >= personSt + personN)
//             break;
//     }
// }
    
// cout<<sum * 1.0 / personN<<endl;
// exit(0);
// //测试person平均度
    ...
    Que[0]->BFSQ = new BFSQue;
    Que[0]->BFSQ->vec = (BFSNode*)malloc(sizeof(BFSNode) * total_v);
    memset(Que[0]->BFSQ->vec, 0 , total_v * sizeof(BFSNode));
    Que[0]->BFSQ->tmp = (BFSNode**)malloc(sizeof(BFSNode*) * (max_threads + 1));
    for(u64 i = 0;i < max_threads + 1;++i){
        Que[0]->BFSQ->tmp[i] = (BFSNode*)malloc(sizeof(BFSNode) * (Q_size >> 3));
        memset(Que[0]->BFSQ->tmp[i], 0, sizeof(BFSNode) * (Q_size >> 3));
    }

    Que[0]->pageR = new PageR();
    Que[0]->pageR->pageRankVec1 = new double[total_v];
    Que[0]->pageR->pageRankVec2 = new double[total_v];
    double pageRankInitValue = 1.0 / total_v;
    for(u64 i = 0;i < total_v;++i)
        Que[0]->pageR->pageRankVec1[i] = pageRankInitValue;
...
totaltime.tv_usec += (etime.tv_usec - btime.tv_usec);
        ...
        if((mod >= 3 && mod <= 7) || (mod >= 11 && mod <= 13)){
            ...
            memset(Que[0]->BFSQ->vec, 0 , Que[0]->BFSQ->tidx * sizeof(BFSNode));
            // for(u64 i = 0;i < max_threads + 1;++i)
            //     memset(Que[0]->BFSQ->tmp[i], 0, sizeof(BFSNode) * (Q_size >> 3));
        }
        else if(mod == 15){
...
if((i + 1) % 10 == 0)
fprintf(stderr,"Com : %-5d\n",(i + 1));
        ++i;
    }
    fin.close();

    ofstream fout;
    fout.open(objfn.c_str(), ios_base::out | ios_base::app);
    fout<<"Mod "<<mod<< "Total Time : "<<totaltime.tv_sec * 1000000 + totaltime.tv_usec<<" us"<<endl;
    fout<<"Arg time : "<<(totaltime.tv_sec * 1000000 + totaltime.tv_usec) / i<<" us"<<endl;
    fout.close();

}

void Graph::TestSerch(int mod, string obj)
{
    switch (mod)
    {
...
void Graph::BFS(string obj)
{
    string type = obj.substr(0, obj.find_first_of(' '));
    u64 id = strtoul(obj.substr(obj.find_first_of(' ') + 1).c_str(), NULL, 10);
    /*
        这个算不算一次查询时间里面
    */
    InArray *vis = Que[0]->vis;
    BFSQue *BFSQ = Que[0]->BFSQ;
    u64 realId;
    GetRealId(type, id, realId);
    BFSQ->hidx = 0;
    BFSQ->tidx = 0;
    new (&BFSQ->vec[BFSQ->tidx++]) BFSNode(realId, 1, NULL);
    vis->SetValue(realId, 1);
    while(BFSQ->hidx < BFSQ->tidx){
        u64 sumWork = max_threads + 1;
        u64 curWork = 0;
        u64 price = (BFSQ->tidx - BFSQ->hidx) / max_threads;
        u64 mod = (BFSQ->tidx - BFSQ->hidx) % max_threads;

        for(u64 i = 0;i < max_threads;++i){
            BFS_Arg *tmp1 = new BFS_Arg(BFSQ, BFSQ->tmp[i], &curWork, this, Que[0]->tasklock, vis);
            tmp1->st = i * price + BFSQ->hidx;
            tmp1->n = price;
            pool->addwork(tmp1, BFS_Fun);
        }
        BFS_Arg *tmp2 = new BFS_Arg(BFSQ, BFSQ->tmp[max_threads], &curWork, this, Que[0]->tasklock, vis);
        tmp2->st = max_threads * price + BFSQ->hidx;
        tmp2->n = mod;
        pool->addwork(tmp2, BFS_Fun);
        ...
        BFSQ->hidx = BFSQ->tidx;
        for(u64 i = 0;i < sumWork;++i){
            for(u64 j = 0;BFSQ->tmp[i][j].id != -1;++j){
                if(!vis->GetValue(BFSQ->tmp[i][j].id)){
                    vis->SetValue(BFSQ->tmp[i][j].id, 1);
                    BFSQ->vec[BFSQ->tidx++] = BFSQ->tmp[i][j];
                }
            }
        }
    }
...
void Graph::PageRank(string obj)
{
    double d = strtod(obj.substr(0, obj.find_first_of(' ')).c_str(), NULL);
    u64 K = strtoul(obj.substr(obj.find_first_of(' ') + 1).c_str(), NULL, 10);
    double *preVec = NULL, *nowVec = NULL;
    u64 tmp = total_v / PAGERANK_NUMS_ONETHREAD;
    u64 sumWork = tmp + 1;
    u64 price = PAGERANK_NUMS_ONETHREAD;
    u64 mod = total_v % PAGERANK_NUMS_ONETHREAD;
    for(u64 i = 0;i < K;++i){
        u64 curWork = 0;
        if(Que[0]->pageR->flag == 1){
            preVec = Que[0]->pageR->pageRankVec1;
            nowVec = Que[0]->pageR->pageRankVec2;
            Que[0]->pageR->flag = 2;
        }
        else{
            preVec = Que[0]->pageR->pageRankVec2;
            nowVec = Que[0]->pageR->pageRankVec1;
            Que[0]->pageR->flag = 1;
        }
        for(u64 j = 0;j < tmp;++j){
            PageRank_Arg *tmp1 = new PageRank_Arg(preVec, nowVec, Que[0]->tasklock, &curWork, this);
            tmp1->d = d;
            tmp1->st = j * PAGERANK_NUMS_ONETHREAD;
            tmp1->n = PAGERANK_NUMS_ONETHREAD;
            pool->addwork(tmp1, PageRank_Fun);
        }
        PageRank_Arg *tmp2 = new PageRank_Arg(preVec, nowVec, Que[0]->tasklock, &curWork, this);
        tmp2->d = d;
        tmp2->st = tmp * PAGERANK_NUMS_ONETHREAD;
        tmp2->n = mod;
        pool->addwork(tmp2, PageRank_Fun);
        while(1)
        {
            {
                MutexLockGuard tmut(*Que[0]->tasklock);
