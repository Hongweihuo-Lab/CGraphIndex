...
void PreProcess::HandleVertex()
{
    ...
    for(u64 i = 0;i < vertexfiles.size();++i) 
    {
        ...
        tvmap.second.bphf = new boomphf::mphf<uint64_t, hasher_t>(n, data_iterator, max_threads, gammaFactor);
        vmap.insert(tvmap);
        delete [] tdata;
        /* 顶点映射完毕 */
        /* 写到文件 */
...
void PreProcess::HandleEdge()
{
    ...
    std::vector<InArray *> thasdes(totale, nullptr);
    std::vector<InArray *> tetype(totale, nullptr);
