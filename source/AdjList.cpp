...
void AdjList::construct_v2() {
    for (uint64_t adj_buf_reader = 0; adj_buf_reader < adjlen; ) {
        uint64_t vid = adj[adj_buf_reader];
        uint64_t len = adj[adj_buf_reader + 1];
        std::span<u64> out_vids{adj + adj_buf_reader + 2, len}; 
        adj_buf_reader += 2 + len;

        if (!out_vids.empty()) {
            
            //simdmaxbitsd1(0, reinterpret_cast<uint32_t *>())
        } 
    }
}
...
std::vector<u64> AdjList::GetNextVid(u64 id) {
    if(id >= Vnums) {
        cerr<<"Max V is:"<<'\t'<<Vnums<<'\t'<<id<<endl;
        exit(1);
    }
    
    u64 ofs, bits, sam, st, num;
    outlist->ThreeB->GetBitsSam(id, ofs, bits, st, sam, num);
    if(num == 0)
        return {};

    std::vector<u64> result(num);
    result[0] = sam;
...
std::vector<u64> AdjList::GetPrevVid(u64 id) {
    if(id >= Vnums) {
        cerr<<"Max V is:"<<'\t'<<Vnums<<'\t'<<id<<endl;
        exit(1);
    }
    u64 ofs, bits, st, sam, num;
    inlist->ThreeB->GetBitsSam(id, ofs, bits, st, sam, num);
    if(num == 0)
        return {};
...
u64 AdjList::__get_input_neighbors(u64 id, u64 *output_buf, u64 buf_size) {
    if(id >= Vnums) {
        cerr<<"Max V is:"<<'\t'<<Vnums<<'\t'<<id<<endl;
        exit(1);
    }
    ...
    for(u64 i = 1; i < num; ++i) {
        ...
        output_buf[i] = output_buf[i - 1] + v;
        ofs += bits;
    }
    return num;
}

u64 AdjList::__get_output_neighbors(u64 id, u64 *output_buf, u64 buf_size) {
    if(id >= Vnums) {
        cerr<<"Max V is:"<<'\t'<<Vnums<<'\t'<<id<<endl;
        exit(1);
    }
...
AdjList::__encoding_state AdjList::__iterative_get_input_neighbors(u64 id, u64 n, u64 *output_buf, u64 buf_size) {
    /*if(id >= Vnums) {
        cerr<<"Max V is:"<<'\t'<<Vnums<<'\t'<<id<<endl;
        exit(1);
    }
    ...
    __encoding_state state;

    inlist->ThreeB->GetBitsSam(id, state.ofs, state.bits, state.st, state.sam, state.num);
    if(state.num == 0)
        return state;
...
AdjList::__encoding_state AdjList::__iterative_get_input_neighbors(const __encoding_state &prv_state, u64 id, u64 n, u64 *output_buf, u64 buf_size) {
